...
$env.config = {
    ...
    hooks: {
        ...
        env_change: {
            PWD: [{|before, after| { || if (which direnv | is-empty) {return}
                                        direnv export json | from json | default {} | load-env} }] # run if the PWD environment is different since the last repl input
        }
